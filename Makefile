...
reformat-diff:
	ocamlformat --doc-comments=before --enable-outside-detected-project --inplace $(shell git status -s | cut -c 4- | grep '\.mli\?$$' | while IFS= read -r f; do stat "$$f" >/dev/null 2>&1 && echo "$$f"; done) || true
